...
public class ProductDAO extends HibernateDAO {
	...
	public static List<Products> lstProductPaid(String lang) {
		...
		return HibernateDAO
				.getList(
						"select pro from Inventory inv,Products pro where pro.productId=inv.productId and inv.limitDate > '"
								+ currentDate + "'", lang);
	}
	...
	public static List<Products> SearchProductkey(String key, int ManuId,
		...
		return HibernateDAO.getList("from Products where  productName like '%"
				+ key + "%' or price between " + prMin + " and " + prMax
				+ " or manufacturerId=" + ManuId, lang);
	}
	...
	public static List<Products> lstProduct(int pageNumber, int pageSize,
			String lang) {
		return HibernateDAO
				.getList("from Products", pageNumber, pageSize, lang);
	}
	...
	public static List<Products> lstProductPaid(int pageNumber, int pageSize,
		...
		return HibernateDAO
				.getList(
						"select pro from Inventory inv,Products pro where pro.productId=inv.productId and inv.limitDate > '"
								+ currentDate + "'", pageNumber, pageSize, lang);
	}
	...
	public static List<Products> lstProductNew(String lang) {
		...
		return HibernateDAO
				.getList(
						"select p from Products p,Inventory inv where p.productId=inv.productId and inv.limitDate > '"
								+ currentDate + "' order by p.uploadDate desc",
						0, 12, lang);
	}
	...
	public static List<Products> lstProduct(CategoryChild cc, int pageNumber,
		...
		return HibernateDAO.getList("from Products where categoryChildId='"
				+ cc.getCategoryChildId() + "'", pageNumber, pageSize, lang);
	}
		...
		return HibernateDAO
				.getList(
						"select pro from Inventory inv,Products pro where pro.productId=inv.productId and inv.limitDate > '"
								...
								+ "' "
								+ " and pro.categoryChildId='"
								+ cc.getCategoryChildId() + "'", pageNumber,
						pageSize, lang);
	}
		...
		return HibernateDAO
				.getList(
						"select pro from Inventory inv,Products pro where pro.productId=inv.productId and inv.limitDate > '"
								+ currentDate
								+ "'"
								+ " and pro.categorySubId='"
								+ cb.getCategorySubId() + "'", lang);
	}
		...
		return HibernateDAO
				.getList(
						"select pro from Inventory inv,Products pro where pro.productId=inv.productId and inv.limitDate > '"
								...
								+ "'"
								+ " and pro.categorySubId='"
								+ cb.getCategorySubId() + "'", pageNumber,
						pageSize, lang);
	}
	...
	public static List<Products> lstProduct_PromotionNews(String lang) {
		...
		List<Promotions> lst = HibernateDAO.getList(
				"from Promotions order by promotionId desc", 0, 10, lang);
	...
	public static double maxPrice(String lang) {
		List kq = HibernateDAO.getList("from Products order by price desc",
				lang);
	...
	public static double minPrice(String lang) {
		List kq = HibernateDAO
				.getList("from Products order by price asc", lang);
		if (kq != null && kq.size() > 0) {
			Products p = (Products) kq.get(0);
			return p.getPrice();
		}
	...
	// Searchproduct with name
	public static List<Products> SearchProductkey(String key, int ManuId,
			Double min, Double max, int pageNumber, int pageSize, String lang) {
	...
	public static boolean updateProductAmount(Products product, String lang) {
		...
		try {
			// String
			// sql="update bank set AccountNumber='"+bank.getAccountNumber()+"',Banking='"+bank.getBanking()+",'AccountHolder='"+bank.getAccountholders()+"' where BankId="+bank.getBankId();
			String sql = "update products set Amount=" + product.getAmount()
					+ ",Price=" + product.getPrice() + "where ProductID="
	...
	public static List<Products> SearchProductAmount(String categoryId,
		...
		return HibernateDAO.getList("from Products where categorySubId='"
				+ categorysubId + "' or categoryChildId='" + categorychildId
				+ "' or categoryId='" + categoryId, lang);
	}

	public static ArrayList<Products> lstProduct1(String username,
			String categorychild, String categorysub, String lang) {
		...
		try {
			String sql = "select products.ProductID,products.Amount,products.ProductName,products.Price "
					...
					+ "and (categorychild.CategoryChildId='"
					+ categorychild
					+ "' or  categorysub.CategorySubId='"
					+ categorysub
					+ "')"
	...
	public static ArrayList<Products> lstProduct2(String username,
			String categorychild, String lang) {
		ArrayList<Products> lstProduct = new ArrayList<Products>();
		MySqlDataAccessHelper helper = new MySqlDataAccessHelper();
		try {
			String sql = "select products.ProductID,products.Amount,products.ProductName,products.Price "
	...
	public static List<Products> getListProductByCategory(String categoryID,
		...
		List<CategoryChild> listCategoryChilds = CategoryChildBUS
				.lstCategoryChild(categoryID, lang);

		List<Products> list = HibernateDAO
				.getList(
	...
	public static Map getListProductByCategoryPaging(String categoryID,
			int firstResult, int maxResult, String lang) {
		SimpleDateFormat simp = new SimpleDateFormat("yyyy-MM-dd");
		String currentDate = simp.format(new Date());
